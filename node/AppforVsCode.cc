    ...
    switch (pk->getPacketType()) {
        case 0: { // Transaction Request Received
            ...
            if (tempBlockID != -1 || transactionStage != 0) { //currently in another transaction
                                                              //tempBlockID中存放的是目的地节点的地址，默认值是-1，如果不是-1则表明正在与其它节点进行交易;
                                                              //而transactionStage，在收到交易请求后会置1
                createBusyMessage(pk->getSrcAddr());
                break;
            }

            calculateChainValue();//计算得到的是localTotalChainValue，也就是本地的链上所有交易值的总和
            if (isNodeEvil() && chainTotalValue != 0) { //ignore all the incoming transactions//如果本身是恶意节点则忽略掉其它交易
                createBusyMessage(pk->getSrcAddr());
                break;
            }
            ...
            if((long) par("transaction_limit") > 0 ){
                ...
                if(numberOfTransactionCounter > (long) par("transaction_limit")){//transaction_limit在omnetpp.ini中赋值默认为-1，也就是对数量没有限制
                   numberOfTransactionCounter=0;
                   endSimulation();
                }
            }

            tempBlockID = pk->getSrcAddr();//A向B发起交易时，将tempBlockID赋值为B的地址，B接收到该交易后又将tempBlockID赋值为A的地址
            tempPartnerSeqNum = pk->getMyChainSeqNum();//这里得到的是B的SeqNum
            tempBlockTransaction = pk->getTransactionValue();//TransactionValue是A设置的
        ...
        case 1: { // Chain Request Received
            if (pk->getUserXID() == myAddress) {//userXID中存储的是target，应该就是交易发起者的地址
...
//TRANSACTION INIT
//初始化一则交易，并且向自己发送一条 timerTransactionTimeout消息
void App::initiateNewTransaction()
{
    if (tempBlockID == -1 && transactionStage == 0) { //No transaction are pending
        calculateChainValue();
        if (chainTotalValue > 0) {
            if (isNodeEvil() && totalEvilTransactions >= (int) par("evilNumberOfTransaction")) {//这里没看懂
                return;
            }
            else {
...
void App::createBusyMessage(int destAddress)//创建busy消息
{
    char pkname[40];
...
void App::createTransactionMessage()//创建交易消息，其中包含了恶意节点创建交易消息的行为
{
...
//AUDITING SYSTEM
void App::contactAnonymizers()//用来寻找匿名节点
{
    //Purge nodes that where offline last time
    for (int i = 0; i < anonymizersTracking.size(); i++) {
        if (anonymizersTracking[i].status == 0) { // it did not replied//-1- available 0- request sent 1- contract accepted 2- positive reply
    ...
    for (int i = 0; i < numberOfAnonymizer; i++) {
        ...
        if (destAddresses == tempBlockID) { //it can happen at most one time//现在tempBlockID中存储的应该是交易发起者的地址
...
//调用这一函数有两种情况：1.交易接收方在经过contactanonymizer之后，找到了匿名节点，调用时的参数分别是（匿名节点地址、目标节点地址（即交易发起方地址）、延迟）
//2.匿名节点收到了某个交易接收方的请求，请求的内容是去匿名审计交易发起方，调用时的参数分别是（目标节点地址、目标节点地址、0）
